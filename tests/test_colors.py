...
def test_terminal_background_color_hits_target_luminance_consistently():
    ...
    for name in names:
        ...
        assert abs(luminance - colors._TERMINAL_BG_TARGET_LUMINANCE_LIGHT) < 0.02


def test_terminal_background_color_dark_mode_hits_dark_target_luminance():
    names = ["proj-0", "proj-1", "proj-2", "proj-3", "proj-4", "proj-5", "proj-6", "proj-7"]
    for name in names:
        hex_color = colors.terminal_background_color(name, dark=True)
        r, g, b = (int(hex_color[i : i + 2], 16) / 255 for i in (1, 3, 5))
        luminance = colors._relative_luminance((r, g, b))
        assert abs(luminance - colors._TERMINAL_BG_TARGET_LUMINANCE_DARK) < 0.02


def test_terminal_background_color_dark_mode_is_darker_than_light_mode():
    for name in ("busy-bee", "social-media-optimizer"):
        light = colors.terminal_background_color(name, dark=False)
        dark = colors.terminal_background_color(name, dark=True)
        light_rgb = (int(light[i : i + 2], 16) / 255 for i in (1, 3, 5))
        dark_rgb = (int(dark[i : i + 2], 16) / 255 for i in (1, 3, 5))
        assert colors._relative_luminance(tuple(dark_rgb)) < colors._relative_luminance(tuple(light_rgb))
...
def test_terminal_background_color_caps_saturation():
    ...
    for name in ("busy-bee", "social-media-optimizer", "some-other-project"):
        for dark in (False, True):
            hex_color = colors.terminal_background_color(name, dark=dark)
            r, g, b = (int(hex_color[i : i + 2], 16) / 255 for i in (1, 3, 5))
            _, _lightness, saturation = colorsys.rgb_to_hls(r, g, b)
            cap = colors._TERMINAL_BG_SATURATION_CAP_DARK if dark else colors._TERMINAL_BG_SATURATION_CAP_LIGHT
